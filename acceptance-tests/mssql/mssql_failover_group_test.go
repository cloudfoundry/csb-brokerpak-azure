...
var _ = Describe("MSSQL Failover Group", func() {
	It("can be accessed by an app before and after failover", func() {
		...
		serviceInstance := helpers.CreateServiceInBroker("csb-azure-mssql-failover-group", "small-v2", helpers.DefaultBroker().Name)
		...
		failoverServiceInstance := helpers.CreateServiceInBroker("csb-azure-mssql-fog-run-failover", "standard", helpers.DefaultBroker().Name , failoverParameters(serviceInstance))
